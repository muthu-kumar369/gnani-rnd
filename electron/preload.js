...
const logger = require('./utils/logger'); // Preload can also use the logger
...
contextBridge.exposeInMainWorld('gnani', {
  ...
  // --- Mic Control ---
  startMic: () => {
    logger.info('Preload calling start-mic in main process.');
    ipcRenderer.send('start-mic');
  },
  stopMic: () => {
    logger.info('Preload calling stop-mic in main process.');
    ipcRenderer.send('stop-mic');
  },
  ...
  vad: {
    ...
    on: (event, callback) => {
      // VAD-specific events
      const validVadEvents = ['audio:listening', 'audio:chunk', 'audio:ended', 'vad:status'];
