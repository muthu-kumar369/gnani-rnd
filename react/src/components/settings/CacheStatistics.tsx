        ...
        <div className="space-y-4">
            <div className="flex items-center justify-between">
                ...
                <button
                    ...
                    className="px-3 py-1.5 bg-red-500/20 hover:bg-red-500/30 text-red-300 rounded-lg transition-colors flex items-center gap-2 text-sm"
                >
                    <Trash2 size={14} />
                    Clear Cache
                </button>
            ...
            <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-black/40 border border-cyan-500/20 rounded-lg p-4">
                    <div className="text-sm text-cyan-400/60 mb-1">Cache Size</div>
                    <div className="text-2xl font-bold text-cyan-100">
                        {stats.size} <span className="text-sm text-cyan-400/60">/ {stats.maxSize}</span>
                    </div>
                ...
                {/* Hit Rate */}
                <div className="bg-black/40 border border-cyan-500/20 rounded-lg p-4">
                    <div className="text-sm text-cyan-400/60 mb-1 flex items-center gap-1">
                        <TrendingUp size={14} />
                        Hit Rate
                    </div>
                    <div className="text-2xl font-bold text-cyan-100">
                ...
                {/* Total Requests */}
                <div className="bg-black/40 border border-cyan-500/20 rounded-lg p-4">
                    <div className="text-sm text-cyan-400/60 mb-1">Total Requests</div>
                    <div className="text-2xl font-bold text-cyan-100">
                ...
                {/* Performance Impact */}
                <div className="bg-black/40 border border-cyan-500/20 rounded-lg p-4">
                    <div className="text-sm text-cyan-400/60 mb-1">Performance Impact</div>
            ...
            {stats.entries.length > 0 && (
                <div className="bg-black/40 border border-cyan-500/20 rounded-lg p-4">
                    <div className="text-sm text-cyan-400/60 mb-3">Cached Conversations</div>
                    <div className="space-y-2 max-h-48 overflow-y-auto custom-scrollbar">
                        {stats.entries.map((entry, index) => (
                            <div
                                key={entry.conversationId}
                                className="flex items-center justify-between text-xs p-2 bg-black/40 rounded"
                            >
                                <div className="flex-1 truncate text-cyan-100">
            ...
            <div className="text-xs text-cyan-400/60 bg-cyan-500/5 border border-cyan-500/10 rounded-lg p-3">
